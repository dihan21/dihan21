👋 Hey, I'm Md. Akib Al Mahmud

🎯 Aspiring Software Engineer | Full-Stack Developer | Problem Solver

🚀 About Me
💡 Passionate about building real, impactful systems — from OS-level programs in C to modern full-stack web apps.
🛠️ Comfortable working with Next.js, React, Tailwind, PostgreSQL, MySQL, PHP, and more.
🌱 Love exploring new technologies, improving system design, and creating tools that help people.
👥 Experienced in leadership roles through university clubs and volunteering.

🛠️ Tech Stack
Languages:
🧑‍💻 C • Python • Java • PHP • JavaScript
Tools & Tech:
🗄️ PostgreSQL • MySQL • Prisma • Drizzle
🌐 Next.js • React.js • Tailwind • ShadCN • Clerk
🛠️ GitHub • VS Code • Figma • Trello
🌐 Stripe • Vonage
🖧 Cisco Packet Tracer
